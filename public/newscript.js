...
function openModal(e) {
  ...
  const imageUrl = e.getAttribute("data-image"); // Ambil URL gambar
  modalBody.innerHTML = "";
...
cover.addEventListener("click", () => {
  iframe.src = iframe.dataset.src; // load video dan autoplay
  cover.style.opacity = "0";
  setTimeout(() => (cover.style.display = "none"), 300); // animasi fade out
});

function initMTCGalleryLightbox() {
  // Buat elemen lightbox jika belum ada
  let lightbox = document.getElementById("mtc-lightbox");

  if (!lightbox) {
    lightbox = document.createElement("div");
    lightbox.id = "mtc-lightbox";
  ...
  const galleryImages = document.querySelectorAll(".mtc-gallery img");
  let currentIndex = 0;
  ...
  // Event listeners untuk setiap gambar
...
document.addEventListener("DOMContentLoaded", function () {
  // Cek apakah ada element .mtc-gallery di halaman
  if (document.querySelector(".mtc-gallery")) {
    initMTCGalleryLightbox();
  }
});
